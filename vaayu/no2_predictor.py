...
class no2_predictor:
    ...
    def get_wide_deep_prediction(self, data):
        ...
        deep_scaler = joblib.load('saved_scalers/no2/no2_wide_deep_deep_scaler.save')
        wide_scaler = joblib.load('saved_scalers/no2/no2_wide_deep_wide_scaler.save')
        
        X_deep = deep_scaler.transform(X_deep)
        X_wide = wide_scaler.transform(X_wide)
        
        model = keras.models.load_model('saved_models/no2/no2_wide_deep.h5')
    ...
    def get_gru_prediction(self, data):
        ...
        scaler = joblib.load('saved_scalers/no2/no2_gru_scaler.save')
        X = scaler.transform(X)
        model = keras.models.load_model('saved_models/no2/no2_GRU.h5')
        X = np.reshape(X, (X.shape[0], 1, X.shape[1]))
